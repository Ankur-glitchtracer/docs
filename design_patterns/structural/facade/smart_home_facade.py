# The "Smart Home" Facade
# Imagine you have a complex home theater system. To watch a movie, you have to:
# 1. Turn on the Projector.
# 2. Set the Projector input to "Netflix."
# 3. Turn on the SoundSystem.
# 4. Set the SoundSystem volume to 20.
# 5. Dim the SmartLights.

# Without a Facade, the user has to call 5+ methods every single time.

# Your Task: The "Movie Night" Button
# 1. The Complex Subsystems:
# Create a Projector class with on() and set_input(source).
# Create a SoundSystem class with on() and set_volume(level).
# Create a Lights class with dim(percent).

# 2. The Facade:
# Create a HomeTheaterFacade class.
# In its __init__, it should receive instances of the three classes above.
# Create a single method: watch_movie(movie_name). This method should coordinate all the steps above in the correct order.
# (Optional) Create an end_movie() method to shut everything down.

# 3. The Goal: The client code should only ever interact with HomeTheaterFacade.

# The Challenge
# While you code the Facade, think about this: Does the Facade "own" the subsystems, or should they be passed in? 
# Usually, we pass them in (Dependency Injection) so that the Facade is even more flexible.

# Can you build this "one-click" movie experience?
