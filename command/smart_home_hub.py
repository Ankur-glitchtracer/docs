# This allows for features like undo/redo, queuing, and macro-recording.

# The Problem: The Programmable Smart Home Hub
# You are building a backend for a Universal Smart Home Remote. 
# This remote needs to control various devices from different manufacturers (e.g., a Philips Light, a Sony TV, and a Nest Thermostat).

# The Requirements
# Uniform Interface: The remote has 4 slots for buttons. Any device action (Turn Light On, Dim Light, Open Garage, Mute TV) should be assignable to any button.
# The "Undo" Button: The remote must have a single "Undo" button that reverses the last command executed, regardless of what that command was.
# Macro Commands: You need to support a "Party Mode" button. When pressed, it should execute a sequence of commands (e.g., Dim Lights, Turn on Music, Set Thermostat to 72°F) in one click.

# Technical Constraints
# The Devices (Receivers): Create classes for Light, Stereo, and TV. Each should have different method names (e.g., the Light has on()/off(), but the Stereo has setVolume() and playCd()).
# The Remote (Invoker): This class should not know anything about how a Light works. It should only know that it has a list of "Command" objects it can trigger.

# Your Implementation Checklist
# To get the most out of this exercise, try to structure your code with these four components:
# The Command Interface: Usually a simple interface with an execute() and an undo() method.
# Concrete Commands: Classes like LightOnCommand or StereoIncreaseVolumeCommand that wrap the device and the specific logic.
# The Invoker: Your RemoteControl class that holds the command objects.
# The Receiver: The actual hardware classes (Light, TV) that perform the work.

# Why this works
# By the time you finish, you’ll see that the RemoteControl doesn't change when you add a new device; you just create a new Command class. 
# That’s the "Aha!" moment of the pattern.
